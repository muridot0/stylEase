...
export default React.memo(function ModelNode({
  ...
  const [styleNodeConnected, setStyleNodeConnected] = React.useState(true)
  //TODO: add the fonts for the card contents and finish functionality
  return (
    <WrapperNode
    ...
    >
      ...
      <div>
        <NodeHandle id='style-input' type='target' position={Position.Left} />
        <NodeHandle
          id='content-input'
          className='!top-[75%]'
          type='target'
      ...
      <div className='text-left'>
        <p
          className={clsx('-top-1 relative', {
            'text-[--node-border-color] dark:text-[--node-icons-color]':
              !styleNodeConnected
          })}
        >
          {!styleNodeConnected ? 'Attach a style node ...' : 'Style node'}
        </p>
        <p className='top-2 relative font-normal text-[--node-border-color] dark:text-[--node-icons-color]'>
